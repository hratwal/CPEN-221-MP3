package ca.ubc.ece.cpen221.mp4.commands;

import ca.ubc.ece.cpen221.mp4.Location;
import ca.ubc.ece.cpen221.mp4.Util;
import ca.ubc.ece.cpen221.mp4.World;
import ca.ubc.ece.cpen221.mp4.ai.PredatorAI;
import ca.ubc.ece.cpen221.mp4.ai.VehicleAI;
import ca.ubc.ece.cpen221.mp4.items.Item;
import ca.ubc.ece.cpen221.mp4.items.LivingItem;
import ca.ubc.ece.cpen221.mp4.items.animals.Velociraptor;
import ca.ubc.ece.cpen221.mp4.items.vehicles.Motorcycle;
import ca.ubc.ece.cpen221.mp4.items.vehicles.Tank;
import ca.ubc.ece.cpen221.mp4.items.vehicles.Truck;

public class SpawnRaptorCommand implements Command{
    private final Item item;
    private final Location target;

    /**
     * Constructor where <code>item</code> is the LivingItem that is breeding
     * and <code>target</code> is the location where the child will appear. The
     * child must be born at an empty location adjacent to the parent.
     *
     * @param item
     *            the parent LivingItem
     * @param target
     *            the location where child will appear
     */
    public SpawnRaptorCommand(Item item, Location target) {
        this.item = item;
        this.target = target;
    }

    @Override
    public void execute(World world) throws InvalidCommandException {
        if (!Util.isValidLocation(world, target) || !Util.isLocationEmpty(world, target)) {
            throw new InvalidCommandException("Invalid SpawnRaptorCommand: "
                    + "Invalid/non-empty raptor target location");
        }
        Velociraptor raptor = new Velociraptor(new PredatorAI(), target);
        raptor.moveTo(target);
        world.addItem(raptor);
        world.addActor(raptor);
        item.loseEnergy(40);
    }
}